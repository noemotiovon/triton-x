...
# Commit: 68a74894b178e5f4ddae518c3e3d658f1aa4e8c2
# Date: 2026-03-18
...
def test_correctness(bsz, seq_len, hidden_size, intermediate_size, dtype, atol, rtol):
    ...
    if dtype == torch.bfloat16 and device == "npu":
        _test_correctness_quack_method(bsz, seq_len, hidden_size, intermediate_size, config, LlamaMLP)
    else:
        _test_correctness_direct(bsz, seq_len, hidden_size, intermediate_size, dtype, atol, rtol, config, LlamaMLP)


def _test_correctness_quack_method(bsz, seq_len, hidden_size, intermediate_size, config, LlamaMLP):
    ...
    llama_mlp_fp32 = LlamaMLP(config=config).to(device).to(torch.float32)
    ...
    llama_mlp_bf16 = LlamaMLP(config=config).to(device).to(torch.bfloat16)
    ...
    liger_mlp_bf16 = LigerGEGLUMLP(config=config).to(device).to(torch.bfloat16)
...
def _test_correctness_direct(bsz, seq_len, hidden_size, intermediate_size, dtype, atol, rtol, config, LlamaMLP):
    ...
    liger_mlp = LigerGEGLUMLP(config=config).to(device).to(dtype)
